...
public class AttendLog {
	...
	public String getDisplay_Attend_Log(@RequestParam(name = "search", required = false) String searchValue,
			Model model) {
		...
		if (searchValue != null && !searchValue.isEmpty()) {
			...
			// SQL文を実行（プレースホルダーを使用）
			String sqlQuery = "SELECT * FROM attendlog WHERE " + whereClause;
	...
	@RequestMapping(path = "/attendlogdele", method = RequestMethod.GET)
	public String getAttend_LogDele(@RequestParam("user_name") String user_name, @RequestParam("day") String day,
			Model model) {
		jdbcTemplate.update("delete from attendlog where user_name = ? and day = ?", user_name, day);
		return "redirect:/attendlog";
	}
